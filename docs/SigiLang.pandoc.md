...
The type $s = \tvar{a}, \tvar{b} \to \tvar{a}$ is more general than $t = \tvar{c}, \tvar{c} \to \tvar{c}$, because $s$ can accept more input types.
That means it should hold that $s <: t$.
To show this, we define a substitution $\theta$ that maps both $\tvar{a}$ and $\tvar{b}$ to $\tvar{c}$.
...
Note that a function type $(c)_n \rightarrow (p)_m$, where no $c_i$ or $p_i$ is a row variable, is canonicalised to $\tvar{S}, (c)_n \rightarrow \tvar{S}, (p)_m$.
